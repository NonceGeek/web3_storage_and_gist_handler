defmodule Web3StorageAndGistCmdTool.Http do
    @retries 5
    @default_user_agent "gist_handler"
    ...
    def http_post(url, data, headers) do
      http_post(url, data, headers, @retries)
    end

    def http_post(_url, _data, _headers, retries) when retries == 0 do
      {:error, "POST retires #{@retries} times and not success"}
    end

    def http_post(url, data, headers, retries) do
      body = Poison.encode!(data)

      url
      |> HTTPoison.post(
        body,
        [{"User-Agent", @default_user_agent}, {"Content-Type", "application/json"}] ++ headers,
        hackney: [headers: [{"User-Agent", @default_user_agent}]]
      )
      |> handle_response()
      |> case do
